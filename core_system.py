...
from config.airbnb_config import FIELD_CATEGORIES
...
class QueryUnderstandingEngine:
    ...
    def _load_nlp_models(self):
        ...
        try:
            ...
            if SentenceTransformer is not None:
                ...
                logger.info("Sentence transformer model loaded for query understanding")
    ...
    def _compute_intent_embeddings(self):
        ...
        intent_examples = {
            'search': [
                ...
                'get accommodations', 'look for rentals'
            ],
            'filter': [
    ...
    def analyze_query(self, query: str, session_context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                'query_expansion': self._extract_query_expansion(processed_query),
            ...
            logger.error(f"Error in advanced query processing: {e}, falling back to basic processing")
        ...
        analysis['query_expansion'] = self._expand_query_nlp(analysis['keywords'])
    ...
    def _classify_intent_nlp(self, query: str) -> Tuple[str, float]:
        ...
        try:
            ...
            logger.error(f"Error in NLP intent classification: {e}")
            return self._classify_intent_pattern(query), 0.5
    ...
    def _enhance_with_context_nlp(self, query: str, context: str) -> str:
        ...
        if self.nlp_model:
            try:
                ...
                logger.error(f"Error in NLP context enhancement: {e}")
        
        # Fallback to keyword-based context enhancement
        return self._enhance_with_context(query, context)
    
    def _expand_query_nlp(self, keywords: List[str]) -> List[str]:
        ...
        if self.sentence_model and keywords:
            try:
                ...
                logger.error(f"Error in NLP query expansion: {e}")
...
class AIJSONSummarizer:
    ...
    def _load_summarization_model(self):
        ...
        try:
            ...
            logger.error(f"Error loading AI summarization model: {e}")
...
class AirbnbDataOptimizer:
    ...
    def categorize_query(self, query: str) -> Dict[str, Any]:
        ...
        category_keywords = {
            ...
            'pricing': ['price', 'cost', 'cheap', 'expensive', 'budget', 'affordable', 'rate', 'fee', 'money'],
        ...
        for key in fields_to_process:
            ...
            try:
                ...
                if isinstance(value, (int, float, bool)):
                    ...
                    clean_str = ''.join(char for char in value if ord(char) < 128)
                    if clean_str.strip():
                    ...
                    for item in value:
                        if item is not None:
                            ...
                            clean_item = ''.join(char for char in item_str if ord(char) < 128)
                            if clean_item.strip():
                    ...
                    try:
                        ...
                        if isinstance(field_constraints, dict):
                            ...
                            for constraint_type, constraint_value in field_constraints.items():
                                ...
                                elif constraint_type == 'min' and value >= target_value * 0.9:  # 90% of min value
                                    constraint_match_score += 1
                                elif constraint_type == 'max' and value <= target_value * 1.1:  # 110% of max value
                                    constraint_match_score += 1
                                else:
                                    field_matches = False
                        else:
...
class JSONRAGSystem:
    ...
    def initialize_system(self) -> bool:
        ...
        logger.info("Initializing JSON RAG System with full enhancements...")
        ...
        try:
            ...
            if self.vocabulary_manager.load_vocabulary():
                logger.info("Vocabulary manager loaded from saved files")
            else:
                logger.warning("No saved vocabulary found, vocabulary features will be limited")
            ...
            if hasattr(self.query_engine, 'airbnb_optimizer'):
                ...
                if not self.vocabulary_manager.vocabulary:
                    ...
                    if documents:
                        ...
                        logger.info("AirbnbOptimizer initialized with MongoDB data as fallback")
                else:
                    # Use loaded vocabulary with optimizer
                    self.query_engine.airbnb_optimizer.vocabulary_manager = self.vocabulary_manager
                    logger.info("AirbnbOptimizer enhanced with loaded vocabulary")
            
            self.system_initialized = True
            logger.info("JSON RAG System with comprehensive enhancements initialized successfully")
            return True
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Could not check database connection: {e}")
        ...
        try:
            ...
            logger.warning(f"Could not get vocabulary statistics: {e}")
        ...
        try:
            ...
            logger.warning(f"Could not check NLP models: {e}")
    ...
    def _apply_intelligent_constraints(self, search_results: List[Tuple[Dict[str, Any], float]], 
        ...
        try:
            ...
            if len(filtered_results) < max(1, len(search_results) * 0.1):
                logger.info("Applying lenient constraint filtering")
            ...
            logger.warning(f"Error applying constraints: {e}")
            return search_results